...
//https://ptsjs.org/demo/edit/?name=guide.sound_visual
  ...
  navigator.mediaDevices
    ...
    .then((stream) => {
      if (stream.getAudioTracks().length > 0) {
        ...
        var sound = Sound.from(source, source.context).analyze(bins);
        console.log(sound);
        ...
        function playButton() {
          if (sound) {
            ...
            form
              .fillOnly("#fff")
              .polygon(
                Triangle.fromCenter([25, 25], 10).rotate2D(Const.half_pi, [
                  25,
                  25,
                ])
              );
          ...
          animate: (time, ftime) => {
            if (sound) {
                  ...
                  (t, i) =>
                    new Group(
                      [
                        t.x,
                        center.y -
                          t.y * Num.cycle(i / bins) * (0.5 + 1.5 * f_scale),
                      ],
                      [
                        t.x,
                        center.y +
              ...
              let eyeLeft = center
                .clone()
                .toAngle(
                  -Const.quarter_pi - Const.half_pi + 0.2,
                  radius / 2,
                  true
                );
              form
